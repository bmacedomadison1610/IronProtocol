<img width="941" height="1672" alt="ChatGPT Image 6 de jun  de 2026, 13_30_00" src="https://github.com/user-attachments/assets/1be6d8c6-8392-4bca-9281-6d67aecb2d7b" />

<img width="353" height="776" alt="image" src="https://github.com/user-attachments/assets/ac59355e-59aa-49c2-b065-16ed1b146e0f" />

<img width="352" height="776" alt="image" src="https://github.com/user-attachments/assets/bcb96df5-889d-46f3-a88e-311b309a72e6" />

<img width="355" height="778" alt="image" src="https://github.com/user-attachments/assets/a908fe27-23e6-4baa-8bca-03784cba5335" />

<img width="357" height="778" alt="image" src="https://github.com/user-attachments/assets/b95bb20a-e944-43a7-b3d4-7e398c33accc" />

<img width="358" height="776" alt="image" src="https://github.com/user-attachments/assets/6f301c78-9dfd-44a0-961d-645c205004fa" />


IRON PROTOCOL

Disciplina. Execução. Resultados.

O Iron Protocol foi desenvolvido para quem busca evolução física real através de um método estruturado de treinamento, nutrição e acompanhamento de resultados.

Organize sua rotina, acompanhe sua dieta, registre sua evolução corporal e mantenha o foco nos seus objetivos de hipertrofia, emagrecimento ou recomposição corporal.

PRINCIPAIS RECURSOS

🏋️ Treinos Estruturados

Organização por dias da semana
Exercícios detalhados
Séries, repetições e descanso
Planejamento completo de treino

🥗 Controle Alimentar

Cadastro personalizado de alimentos
Edição de quantidades e calorias
Controle por refeições
Cálculo automático de calorias

📊 Acompanhamento de Progresso

Registro de peso corporal
Histórico completo de evolução
Monitoramento de metas
Visualização do progresso

🎯 Metas Personalizadas

Peso alvo
Meta calórica
Controle de macronutrientes
Acompanhamento diário

📈 Controle de Consumo

Marcação de refeições concluídas
Checklist de alimentos consumidos
Indicadores de progresso
Acompanhamento em tempo real
PARA QUEM É O IRON PROTOCOL?

✔ Praticantes de musculação

✔ Pessoas em processo de emagrecimento

✔ Atletas amadores

✔ Quem busca ganho de massa muscular

✔ Usuários que desejam mais disciplina e organização

FILOSOFIA DO MÉTODO

Acreditamos que resultados consistentes são construídos através de:

• Disciplina diária

• Treinamento estruturado

• Nutrição estratégica

• Progressão contínua

• Constância nas ações

Resultados não acontecem por acaso.

Construa sua melhor versão.

Categoria

Saúde e Fitness

Palavras-chave (ASO)

hipertrofia, musculação, academia, treino, dieta, fitness, ganho de massa, emagrecimento, nutrição, controle alimentar, personal trainer, treino de musculação, evolução física, fitness tracker, bodybuilding, treino diário

Política de Privacidade (texto resumido)

Política de Privacidade – Iron Protocol

O Iron Protocol coleta apenas as informações necessárias para o funcionamento do aplicativo, incluindo dados de treinos, metas, alimentação e progresso físico registrados pelo usuário.

Os dados são utilizados exclusivamente para fornecer as funcionalidades do aplicativo e melhorar a experiência do usuário.

Não comercializamos informações pessoais com terceiros.

Os dados podem ser armazenados localmente no dispositivo e/ou em serviços seguros de armazenamento em nuvem utilizados pelo aplicativo.

Ao utilizar o aplicativo, o usuário concorda com esta política.
